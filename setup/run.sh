  ...
  for model in ${LLMDBENCH_DEPLOY_MODEL_LIST//,/ }; do
    ...
    if [[ $LLMDBENCH_HARNESS_SKIP_RUN -eq 1 ]]; then
      ...
      if [[ $LLMDBENCH_CONTROL_DRY_RUN -eq 1 ]]; then
        ...
        set +euo pipefail
        if [[ $LLMDBENCH_CONTROL_ENVIRONMENT_TYPE_MODELSERVICE_ACTIVE -eq 1 ]]; then
          received_model_name=$(get_model_name_from_pod $LLMDBENCH_VLLM_COMMON_NAMESPACE $(get_image ${LLMDBENCH_IMAGE_REGISTRY} ${LLMDBENCH_IMAGE_REPO} ${LLMDBENCH_IMAGE_NAME} ${LLMDBENCH_IMAGE_TAG}) ${LLMDBENCH_HARNESS_STACK_ENDPOINT_URL}/${LLMDBENCH_DEPLOY_CURRENT_MODELID} NA)
        else
